    ...
    ILogger<ExpertApprovalService> logger) : IExpertApprovalService
{
        ...
        CancellationToken ct)
    {
        ...
        {
            var row = db.ExpertGroupMembers
                .Where(m => m.UserId == expertUserId && m.Status == ExpertMembershipStatus.Active)
                .Select(m => new { m.ExpertGroupId, m.MemberRole })
                .FirstOrDefault();
            membership = row?.ExpertGroupId ?? Guid.Empty;
            // Sert uniquement à signer l'invitation : un Expert ne doit pas s'annoncer responsable.
            isManager = row?.MemberRole == ExpertGroupMemberRole.Manager;
        }
        if (membership == Guid.Empty)
            throw new InvalidOperationException("Vous n'êtes membre d'aucun groupe d'experts.");
